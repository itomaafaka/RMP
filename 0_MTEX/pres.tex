...
\begin{frame}{Идея: построить портфель, который копирует опцион}

Хотим построить портфель:
\[
\Pi = \Delta S_0 - B_0,
\]
который через один шаг даёт те же выплаты, что и опцион:
...
Это приводит к системе из двух уравнений с двумя неизвестными:
\[
(\Delta, B_0).
\]
...
\begin{frame}{Шаг 1: система уравнений}

Записываем явно:

\[
\Delta u S_0 - B_0(1+r\Delta t)=C_u,
\]
\[
\Delta d S_0 - B_0(1+r\Delta t)=C_d.
\]

Это — две прямые зависимости между $\Delta$ и $B_0$.

\end{frame}

\begin{frame}{Шаг 2: вычитаем уравнения}

Вычитаем второе уравнение из первого:

\[
(\Delta uS_0 - B_0(1+r\Delta t))
-
(\Delta dS_0 - B_0(1+r\Delta t))
= C_u - C_d.
\]

Термины с $B_0$ сокращаются:
...
\begin{frame}{Шаг 3: определяем $B_0$}
Подставляем найденную
\[
\Delta = \frac{C_u - C_d}{(u-d)S_0}
\]
в любое из уравнений, например в верхнее:
\[
\Delta u S_0 - B_0(1+r\Delta t) = C_u.
\]
Тогда:
...
То есть:
\[
B_0 = \frac{\Delta u S_0 - C_u}{1+r\Delta t}.
\]
\end{frame}

\begin{frame}{Шаг 4: стоимость портфеля сегодня}
Стоимость портфеля (в момент $t=0$):
\[
\Pi_0 = \Delta S_0 - B_0.
\]
Так как портфель реплицирует опцион, то:
\[
C_0 = \Pi_0.
\]
Значит:
...
\frac{C_u - C_d}{u-d}
-
\frac{\Delta u S_0 - C_u}{1+r\Delta t}.
\]

\end{frame}

\begin{frame}{Шаг 6: алгебра и общий знаменатель}

Подставим $\Delta S_0 = \frac{C_u - C_d}{u-d}$ внутрь:

\[
\Delta u S_0
=
u \cdot \frac{C_u - C_d}{u-d}.
\]
...
\frac{C_u - C_d}{u-d}
-
\frac{u(C_u - C_d)/(u-d) - C_u}{1+r\Delta t}.
\]

Теперь приводим обе части к знаменателю $(u-d)(1+r\Delta t)$.

После упрощения выходит:

\[
C_0 =
\frac{1}{1+r\Delta t}
\left[
\frac{(1+r\Delta t)-d}{u-d} C_u
+
\frac{u-(1+r\Delta t)}{u-d} C_d
\right].
\]

\end{frame}

\begin{frame}{Шаг 7: вводим риск-нейтральную вероятность}
...
q = \frac{(1+r\Delta t)-d}{u-d},
\qquad
1-q = \frac{u-(1+r\Delta t)}{u-d}.
\]

Получаем красивую формулу:
...
Это и есть цена европейского call в одном шаге биномиальной модели.
...
\begin{itemize}
    \item $q$ — \textbf{не реальная} вероятность роста;
    \item это вероятность, возникающая из требования отсутствия арбитража;
    \item под мерой $q$ дисконтированная цена акции — мартингал;
    \item цена опциона = дисконтированное математическое ожидание payoff.
\end{itemize}

Это основа всей современной теории деривативов.
